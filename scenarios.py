...
class DoorwayScenario:
    def __init__(self):
        ...
        self.goals = np.array([[2, -goal_y, 0.0],
                    [2, goal_y, 0.0]])
        if config.dynamics == DynamicsModel.DOUBLE_INTEGRATOR or config.dynamics == DynamicsModel.DOUBLE_INTEGRATOR_MACBF or config.dynamics == DynamicsModel.INFORMARL:
