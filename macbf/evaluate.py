...
def main():
    ...
    for istep in range(config_macbf.EVALUATE_STEPS):
        ...
        s_np_ori, g_np_ori = core.generate_data(args.num_agents, config_macbf.DIST_MIN_THRES * 1.5)
        ...
        for i in range(config_macbf.INNER_LOOPS):
            ...
            s_ref = np.concatenate([s_np[:, :2] - g_np, s_np[:, 2:]], axis=1)
            a_lqr = -s_ref.dot(K.T)
            s_np = s_np + np.concatenate([s_np[:, 2:], a_lqr], axis=1) * config_macbf.TIME_STEP
        ...
        print('Evaluation Step: {} | {}, Time: {:.4f}'.format(
            istep + 1, config_macbf.EVALUATE_STEPS, end_time - start_time))
