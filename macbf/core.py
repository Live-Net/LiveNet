...
def network_cbf(x, r, indices=None):
    ...
    x, indices = remove_distant_agents(x=x, k=config_macbf.TOP_K, indices=indices)
    dist = tf.sqrt(
        tf.reduce_sum(tf.square(x[:, :, :2]) + 1e-4, axis=2, keepdims=True))
    mask = tf.cast(tf.less_equal(dist, config_macbf.OBS_RADIUS), tf.float32)
...
def network_action(s, g, obs_radius=1.0, indices=None):
    ...
    x, _ = remove_distant_agents(x=x, k=config_macbf.TOP_K, indices=indices)
...
def loss_derivatives(s, a, h, x, r, ttc, alpha, indices=None, eps=[1e-3, 0]):
    ...
    h_next, mask_next, _ = network_cbf(x=x_next, r=config_macbf.DIST_MIN_THRES, indices=indices)
...
def ttc_dangerous_mask(s, r, ttc, indices=None):
    ...
    s_diff, _ = remove_distant_agents(s_diff, config_macbf.TOP_K, indices)
